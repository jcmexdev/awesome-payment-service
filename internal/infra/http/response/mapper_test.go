...
func TestTranslateAppError(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			err:            appErrors.NewAppError(appErrors.TypeValidationError, "INSUFFICIENT_BALANCE", "no money", nil),
			expectedStatus: http.StatusUnprocessableEntity,
			expectedCode:   "INSUFFICIENT_BALANCE",
		},
		{
			name:           "NotFound maps to 404",
			err:            appErrors.NewAppError(appErrors.TypeNotFound, "ACCOUNT_NOT_FOUND", "not found", nil),
			expectedStatus: http.StatusNotFound,
			expectedCode:   "ACCOUNT_NOT_FOUND",
		},
		{
			name:           "Unauthorized maps to 401",
			err:            appErrors.NewAppError(appErrors.TypeUnauthorized, "INVALID_TOKEN", "bad token", nil),
			expectedStatus: http.StatusUnauthorized,
			expectedCode:   "INVALID_TOKEN",
		},
		{
			name:           "Internal error maps to 500",
			err:            appErrors.NewAppError(appErrors.TypeInternal, "DATABASE_ERROR", "db crash", nil),
